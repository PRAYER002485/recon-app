...
const ReconQuery = zod_1.z.object({
    target: zod_1.z.string().trim().toLowerCase().regex(hostnameRegex, 'Invalid domain'),
    mode: zod_1.z.enum(['fast', 'full']).optional(),
});
const UrlScanQuery = zod_1.z.object({
    target: zod_1.z.string().trim().toLowerCase().regex(hostnameRegex, 'Invalid domain'),
    mode: zod_1.z.enum(['fast', 'full']).optional(),
});
// Email query for breach check
const EmailQuery = zod_1.z.object({
...
const reconLimiter = (0, express_rate_limit_1.default)({
    windowMs: 60 * 1000,
    limit: 5,
...
app.post('/api/recon', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const subfinder = (0, child_process_1.spawn)('subfinder', subfinderArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
        const httpx = (0, child_process_1.spawn)('httpx', httpxArgs, { stdio: ['pipe', 'pipe', 'pipe'] });
        ...
        const collectErr = (procName) => (chunk) => { stderr += `[${procName}] ${chunk.toString()}`; };
...
app.post('/api/ports', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const naabu = (0, child_process_1.spawn)('naabu', naabuArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
        let naabuStdout = '';
        let naabuStderr = '';
        const naabuTimer = setTimeout(() => { try {
            naabu.kill('SIGTERM');
        }
        catch { } }, mode === 'full' ? 1000 * 90 : 1000 * 45);
        naabu.stdout.on('data', (chunk) => { naabuStdout += chunk.toString(); });
        naabu.stderr.on('data', (chunk) => { naabuStderr += chunk.toString(); });
        const naabuOk = await new Promise((resolve) => {
        ...
        if (naabuOk) {
            naabuStdout.split('\n').map(l => l.trim()).filter(Boolean).forEach((line) => {
                try {
                    ...
                    if (obj && (obj.host || obj.ip) && typeof obj.port === 'number') {
                        findings.push({ host: obj.host || obj.ip, ip: obj.ip || '', port: obj.port });
                    }
                }
                catch { }
            });
            ...
            const nmap = (0, child_process_1.spawn)('nmap', nmapArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
            let nmapOut = '';
            let nmapErr = '';
            const nmapTimer = setTimeout(() => { try {
                nmap.kill('SIGTERM');
            }
            catch { } }, mode === 'full' ? 1000 * 90 : 1000 * 45);
            nmap.stdout.on('data', (c) => { nmapOut += c.toString(); });
            nmap.stderr.on('data', (c) => { nmapErr += c.toString(); });
            const nmapOk = await new Promise((resolve) => {
        ...
        for (const f of findings) {
            if (!webPorts.has(f.port))
                continue;
            if (f.port === 443 || f.port === 8443)
                urlInputs.push(`https://${f.host}:${f.port}`);
            else
                urlInputs.push(`http://${f.host}:${f.port}`);
        }
        let probes = {};
        if (urlInputs.length > 0) {
            ...
            let httpxStdout = '';
            let httpxStderr = '';
            const httpxTimer = setTimeout(() => { try {
                httpx.kill('SIGTERM');
            }
            catch { } }, mode === 'full' ? 1000 * 90 : 1000 * 45);
            try {
            ...
            httpx.stdout.on('data', (chunk) => { httpxStdout += chunk.toString(); });
            httpx.stderr.on('data', (chunk) => { httpxStderr += chunk.toString(); });
            await new Promise((resolve, reject) => {
                httpx.on('error', reject);
                httpx.on('close', (code) => {
                    ...
                    if (code === 0 || code === null)
                        ...
                        reject(new Error(`httpx exited with code ${code}: ${httpxStderr}`));
                });
            });
            httpxStdout.split('\n').map(l => l.trim()).filter(Boolean).forEach((line) => {
                try {
...
app.post('/api/nmap', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const subfinder = (0, child_process_1.spawn)('subfinder', ['-silent', '-d', target], { stdio: ['ignore', 'pipe', 'pipe'] });
        let sfOut = '';
        let sfErr = '';
        subfinder.stdout.on('data', (c) => { sfOut += c.toString(); });
        subfinder.stderr.on('data', (c) => { sfErr += c.toString(); });
        await new Promise((resolve, reject) => {
            subfinder.on('error', reject);
            subfinder.on('close', (code) => {
                if (code === 0 || code === null)
                    ...
                    reject(new Error(`subfinder exited with code ${code}: ${sfErr}`));
            });
        });
        const hosts = Array.from(new Set(sfOut.split('\n').map(l => l.trim()).filter(Boolean))).slice(0, maxHosts);
        async function scanHost(host) {
            return new Promise((resolve) => {
                ...
                const nmap = (0, child_process_1.spawn)('nmap', args, { stdio: ['ignore', 'pipe', 'pipe'] });
                let out = '';
                let err = '';
                ...
                nmap.stdout.on('data', (c) => { out += c.toString(); });
                nmap.stderr.on('data', (c) => { err += c.toString(); });
                nmap.on('close', () => {
                    clearTimeout(timer);
                    ...
                    if (open.length === 0) {
                        ...
                        const quick = (0, child_process_1.spawn)('nmap', quickArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
                        let qout = '';
                        quick.stdout.on('data', (c) => { qout += c.toString(); });
                        quick.on('close', () => {
...
app.post('/api/js-scan', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const katana = (0, child_process_1.spawn)('katana', katanaArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
        let katanaStdout = '';
        let katanaStderr = '';
        const katanaTimer = setTimeout(() => { try {
            katana.kill('SIGTERM');
        }
        catch { } }, mode === 'full' ? 1000 * 120 : 1000 * 60);
        katana.stdout.on('data', (c) => { katanaStdout += c.toString(); });
        katana.stderr.on('data', (c) => { katanaStderr += c.toString(); });
        await new Promise((resolve, reject) => {
            ...
            katana.on('close', (code) => {
                ...
                if (code === 0 || code === null)
                    ...
                    reject(new Error(`katana exited with code ${code}: ${katanaStderr}`));
            });
        });
        // Filter only .js URLs
        ...
        httpx.stdout.on('data', (c) => { httpxStdout += c.toString(); });
        httpx.stderr.on('data', (c) => { httpxStderr += c.toString(); });
        await new Promise((resolve, reject) => {
            httpx.on('error', reject);
            httpx.on('close', (code) => {
                clearTimeout(httpxTimer);
        ...
        for (const url of toFetch) {
            fetchPromises.push(new Promise((resolve) => {
                const curl = (0, child_process_1.spawn)('curl', ['-sL', '--max-time', mode === 'full' ? '15' : '8', url], { stdio: ['ignore', 'pipe', 'pipe'] });
                let body = '';
                curl.stdout.on('data', (c) => { body += c.toString(); });
                curl.on('close', () => {
    ...
    return new Promise((resolve) => {
        const socket = tls.connect(port, host, {
            ...
            if (cert && Object.keys(cert).length > 0) {
                ...
                result.signatureAlgorithm = cert.sigalg || null;
...
app.post('/api/ssl-check', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        let subfinderStdout = '';
        let subfinderStderr = '';
        const subfinderTimer = setTimeout(() => { try {
            subfinder.kill('SIGTERM');
        }
        catch { } }, mode === 'full' ? 1000 * 90 : 1000 * 60);
        subfinder.stdout.on('data', (chunk) => { subfinderStdout += chunk.toString(); });
        subfinder.stderr.on('data', (chunk) => { subfinderStderr += chunk.toString(); });
        await new Promise((resolve, reject) => {
            subfinder.on('error', reject);
            subfinder.on('close', (code) => {
                ...
                if (code === 0 || code === null)
                    ...
                    reject(new Error(`subfinder exited with code ${code}: ${subfinderStderr}`));
            });
        });
        const subdomains = Array.from(new Set(subfinderStdout.split('\n').map(l => l.trim()).filter(Boolean))).slice(0, mode === 'full' ? 200 : 100);
...
app.post('/api/breach-check', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const rawBreaches = Array.isArray(json.breaches) ? json.breaches : (Array.isArray(json.data) ? json.data : []);
...
app.post('/api/urls-scan', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const subdomains = Array.from(new Set(subfinderStdout.split('\n').map(l => l.trim()).filter(Boolean))).slice(0, mode === 'full' ? 100 : 50);
        // Fetch historical URLs from Wayback Machine for each subdomain
        const waybackPromises = [];
        ...
        const uniqueResults = results.filter((result, index, self) => index === self.findIndex(r => r.url === result.url)).slice(0, mode === 'full' ? 500 : 200);
        res.json({
            target,
...
app.post('/api/headers-check', reconLimiter, async (req, res) => {
    ...
    try {
        ...
        const fetchWithTimeout = (input) => fetch(input, { method: 'GET', redirect: 'manual', headers: { 'User-Agent': 'ReconApp/1.0' }, signal: AbortSignal.timeout(mode === 'full' ? 12000 : 6000) });
        const results = [];
...
app.post('/api/dns-hygiene', reconLimiter, async (req, res) => {
    ...
    const resolveAAAA = (0, util_1.promisify)(dns.resolve6);
    const resolveMX = (0, util_1.promisify)(dns.resolveMx);
    const resolveTXT = (0, util_1.promisify)(dns.resolveTxt);
    const resolveNS = (0, util_1.promisify)(dns.resolveNs);
    const resolveCAA = (0, util_1.promisify)(dns.resolveCaa || ((_h, _cb) => _cb(new Error('CAA not supported'))));
    try {
        ...
        let caa = [];
        try {
            caa = await resolveCAA(target);
        }
        catch { }
        const flatTxt = txt.map(arr => arr.join(''));
        const spf = flatTxt.find(t => /^v=spf1\s/i.test(t)) || null;
        // DMARC lives at _dmarc.<domain>
        let dmarc = null;
        try {
            const dmarcTxt = await resolveTXT(`_dmarc.${target}`);
            const dmFlat = dmarcTxt.map(a => a.join(''));
            dmarc = dmFlat.find(t => /^v=DMARC1;/i.test(t)) || null;
        }
        catch { }
        const hygieneFindings = [];
        hygieneFindings.push({ url: `dns://${target}`, host: target, statusCode: a.length + aaaa.length, title: `A/AAAA: ${a.join(', ')} ${aaaa.join(', ')}`.trim(), technologies: ['A', 'AAAA'] });
        hygieneFindings.push({ url: `dns://${target}/mx`, host: target, statusCode: mx.length, title: mx && mx.length ? `MX ok (${mx.map((m) => `${m.exchange}:${m.priority}`).join(', ')})` : 'No MX records', technologies: ['MX'] });
        ...
        hygieneFindings.push({ url: `dns://${target}/ns`, host: target, statusCode: ns.length, title: ns && ns.length ? `NS: ${ns.join(', ')}` : 'No NS records', technologies: ['NS'] });
    ...
    try {
        ...
        if (vtKey) {
            try {
                ...
                const vtJson = await vtResp.json().catch(() => ({}));
                ...
                results.push({ url: `vt://${target}`, host: target, statusCode: vtResp.status, title: `VT rep=${rep} harmless=${harmless} malicious=${malicious}`, technologies: Object.values(cats) });
            }
            catch (e) {
        ...
        if (gsbKey) {
            try {
                ...
                const gsbJson = await gsbResp.json().catch(() => ({}));
                const matches = Array.isArray(gsbJson?.matches) ? gsbJson.matches : [];
                results.push({ url: `gsb://${target}`, host: target, statusCode: gsbResp.status, title: matches.length ? `GSB matches: ${matches.map((m) => m.threatType).join(', ')}` : 'GSB: no matches', technologies: ['GSB'] });
            }
            catch (e) {
